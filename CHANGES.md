...
- The existing `core_user` subsystem metadata link for `timezone` is unchanged. The new
  ...
  exported and deleted/anonymised by `core_user`'s own privacy provider. `core_userlist_provider`
  was deliberately not added, since the plugin has no context-scoped list of affected users of
  its own to report.
- Added PHPUnit coverage exercising `core_privacy\manager::component_is_compliant()` directly
  (the actual registry-compliance regression, not just interface introspection), plus direct
  coverage of `get_contexts_for_userid()`, `export_user_data()`, `delete_data_for_user()` and
  `delete_data_for_all_users_in_context()` proving the user's timezone and an unrelated core
  profile field are never mutated by this plugin's own request-provider methods.
- Raised `$plugin->release` from `0.1.6` to `1.1` and `$plugin->version` from `2026082006` to
  ...
  #6 remains open. Staging re-verification of the registry-compliance fix itself is pending
  post-merge redeployment.
